...
\begin{definition}
    ...
    \[
        \sigma_D(X,L,J)\vcentcolon=\text{inf}\left\{\langle[\omega],[u]\rangle\middle|u:(\mathbb{D}^2,\partial {\mathbb{D}^2})\to(X,L) \text{ non-constant $J$-holomorphic disc}   \right\}
    \]
    and 
    \[
...
\begin{proof}
  ...
  In particular $( ℂ × T^* S^1, ω_0, J_0)$ is geometrically bounded.\todo{Why?}
  ...
  \[ d(μ^{-1}(u),μ^{-1}(v))^2 ≥ (\sqrt{v_1}-\sqrt{u_1})^2 + (v_2-u_2)^2 ≕ d_{μ_0}(u,v)^2 \; ,\]
  defining us a metric on $\im μ_0$.

  Let $C$ be the constant in \cref{thm:monotonicity} for $( ℂ × T^* S¹, ω_0, J_0)$.
  Set $A_0^- = x, A_0 = B^{d_{μ_0}}_{2\sqrt{\frac{x_1}{C}}}(x)$ and $A_0^+ = ∂A_0$, where $B_r^{d_{μ_0}}(x)$ denotes the metric ball with radius $r$ around $x$ with respect to $d_{μ_0}$.

  Let us now return to $B_{dpq}$. Using a nodal slide we may assume that $A_0$ contains no nodes.
  ...
  Let $J_{B_{dpq}}$ be a geometrically bounded almost complex structure on $B_{dpq}$ (exists by \cref{rem:Bdpq_geometrically_bounded}).
  Let $K$ be a compact set with its interior containing $μ^{-1}(A_0)$. Then there exists a geometrically bounded almost complex structure $J$ (by \cref{rem:J_compactly_perturbed}) on $B_{dqp}$ such that $J=J_{B_{dqp}}$ on $B_{dqp} ∖ K$ and $J=J_0$ on $μ^{-1}(A_0)$.

  Using \cref{thm:small_buffer} with $A^± = μ^{-1}(A_0^±)$, we get that any $J$-holomorphic disk $u\colon (D,∂D) → (B_{dpq}, T(x))$ (after restricting to the part contained in $μ^{-1}(A_0)$) must have 
  \[∫_u ω ≥ \frac{C}{4} d(A^+,A^-)^2 ≥ \frac{C}{4} \frac{4x_1}{C} = x_1 \;. \]

  Noting that there cannot be any $J$-holomorphic spheres (since $H_2(B_{dpq})$ is generated by Lagrangians), and then using \cref{thm:chekanov} we conclude.
\end{proof}
