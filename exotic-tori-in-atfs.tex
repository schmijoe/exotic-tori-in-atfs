...
Note that there may be multiple branch cuts meeting at the same vertex of the polygon $Δ$, see e.g.\ \cref{fig:atf_example2}. It will be useful for us to distinguish between vertices of $\Delta$ and pairs consisting of a vertex and a branch cut. This motivates the following definition.

\begin{definition}
  \label{def:atf_corner}
  An \textbf{ATF-corner} of $Δ$ is a pair $(x,dv)$ consisting of a vertex $x$ and a branch cut intersecting $x$. By $v ∈ ℤ²$, we denote the primitive direction vector along the chosen branch cut which points from $x$ into the interior of $\Delta$. By $d \in \mathbb{N}$, we denote the number of nodes on that branch cut.
