...
As in \cite{EvaUrz21}, the idea of the proof of \cref{thm:toric_embeddings} is to perform an infinite sequence of mutations in $Δ$, to obtain a sequence of partial $B_{d_i p_i q_i}$-embeddings.
In order to avoid some cyclical behaviour in the sequence $(d_i,p_i,q_i)$, we fix an edge and perform mutations at an adjacent vertex, thus elongating the fixed edge.
