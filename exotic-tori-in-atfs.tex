...
\end{proposition}
 ...
 As outlined in \cref{sec:howto_exotic}, combining almost toric fibrations with the displacement energy of its toric fibres allows us to compute the displacement energy germ $[e]_L$ for $L = T^k_{pq}(a)$ on an open dense subset as in \cref{def:our_tori}.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{scope}[shift={(-5,0)}]
      ...
      \draw[thick] (0,-1) -- (0,2);
      \draw[dashed] (1,0.5) node[cross] {} -- (2,1) node[cross] {} -- (3,1.5) node[cross] {} -- (4,2);
      \draw[fill] (2.5,1.25) circle[radius=1pt] node[anchor=south] {$T^k_{pq}$};
    \end{scope}
    \begin{scope}[shift={(1,0)}]
      \draw[fill,opacity=0.2] (0,2) -- (0,0) -- (4,1.2) -- (4,2);
      \draw[thick] (0,2) -- (0,0) -- (4,1.2);
      \draw[dashed] (0,0) -- (1,0.5) node[cross] {} -- (2,1) node[cross] {} (3,1.5) node[cross] {} -- (4,2);
      \draw[fill] (2.5,1.25) circle[radius=1pt] node[anchor=south] {$T^k_{pq}$};
    \end{scope}
  \end{tikzpicture}   
  \caption{Modifiying the alsmot toric moment map $π$.}
  \label{fig:mod_moment_map}
\end{figure}

\begin{proof}
    Let $\pi \colon B_{dpq} \rightarrow \Delta_{dpq}$ be an almost toric moment map with $k$ nodes t
