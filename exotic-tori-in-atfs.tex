...
\begin{proposition}
    Let $\Delta \subset \mathbb{R}^2$ be an almost toric base diagram containing a node $n \in \Delta$. Up to translation, we assume that $n = (0,0) \in \mathbb{R}^2$. Furthermore, let $v \in \mathbb{Z}^2$ be a primitive vector determining the eigenline of the node $n$. Then the almost toric base diagram after a change in branch cut at $n$ is obtained from $\Delta$ by applying the piece-wise linear map 
    \begin{equation}
    \label{eq:half-shear}
        x \mapsto
        \begin{cases}
            S_v(x) = x + \operatorname{det}(x,v)v, & \text{if } \operatorname{det}(x,v) \geqslant 0, \\
            x & \text{if } \operatorname{det}(x,v) < 0.
        \end{cases}
    \end{equation}
\end{proposition}

Since $v$ is an eigenvector of $S_v$, the map \eqref{eq:half-shear} is continuous. Note  
...
\begin{theorem}
  ...
  \begin{equation}
      [e]_{T_{kpq}(a)} (b) \sim a + \min\qty{b_1, b_1 (1-kpq) + b_2 k p^2} \; ,
  \end{equation}
