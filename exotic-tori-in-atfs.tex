...
For a model $X_{P} = M_P / G_p$, we obtain an integrable system $\mathbf{H}_P \colon X_P \rightarrow \mathbb{R}_{\geqslant 0} \times \mathbb{R}$ from which we construct an almost toric projection $\pi_P \colon X_P \rightarrow \Delta_{\pi_P}$ almost toric base $\Delta_{\pi_P} = \mathbb{R}_{\geqslant 0} \times \mathbb{R}$. For $x \in \Delta_{\pi_P}$, denote the corresponding fibre by $T(x) = \mu^{-1}_P(x)$. For every $x$ not contained in the nodal slide line $\ell \subset \Delta_{\pi_P}$, the fibre $T_P(x)$ is a Lagrangian torus.
