...
\begin{definition}
  ...
  \begin{align*}
    ...
    ξ &↦  ∫_{C_ξ} ω
  \end{align*}
  where $C_ξ$ is the cylinder obtained by sliding a representative of $ξ$ in $L_0$ along the Lagrangian isotopy. This is well defined (independent of the choice of $C_ξ$) and also independent by end point preserving isotopies of $Λ$.\footnote{If $C_ξ ≅ C_ξ'$ by homotopy $H$, $0=∫_{∂H} ω = ∫_{C_ξ} ω - ∫_{C_ξ'} ω + ∫_A ω + ∫_B ω$, with $A,B$ contained in the endpoints of the isotopy, which are Lagrangian.}
...
\end{lemma}
\begin{proof}
  Let $γ(θ), θ ∈ S¹$ be a parametrisation of a 1-cycle in $L_0$, $X_t$ a Vector field generating the flow $φ_t$ with $φ_t(L_0) = Λ_t$, and $φ_t(γ(θ))$ a parametrisation of the cylinder $C_γ$ over $Λ_t$ as in \cref{def:flux_map}.
...
\end{lemma}
\begin{proof}
  Let $ξ$ be some 1-cycle in $L_0$, $C_ξ$ the cylinder along $Λ_t$ with $∂C_ξ = α ∘ ξ' - s_0 ∘ ξ$ where $α ∘ ξ'$ is a 1-cylce in $Γ_α$, and $s_0$ the zero section.
...
As above, take a neighbourhood $U ⊂ \symcal{L}$ s.t.\ every $L' ∈ U$ is given as $φ(Γ_α)$ for some $α ∈ Ω_1(L_0)$ and $\symcal{U}$ is convex w.r.t.\ the linear structure coming from $Ω_1(L_0)$.
...
\begin{proposition}
  ...
  The map $ \Flux_{\symcal{U}} \colon \symcal{U} → H¹(L_0;ℝ)$ obtained by restricting $\Flux$ is well defined, and classifies $\symcal{U}$ up to Hamiltonian isotopies contained in $U$.
\end{proposition}
\begin{proof}
  This is essentially \cref{thm:cotangent_classification}. The fact that $\symcal{U}$ is convex guarantees that the linear Hamiltonian isotopy of \cref{thm:graph_hamiltonian_isotopic} between $L_1 ≅ L_2$, both in $\symcal{U}$ exists.\todo{Könnte man convex mit path-connected fibres ersetzen?}.
\end{proof}

\begin{definition}
  A \textbf{versal deformation $v_{L_0}$ of $L_0$} is a "section of all local flux maps", i.e.\ 
  $v_{L_0} \colon H^1(L_0;ℝ) \dashrightarrow \symcal{L}$ \footnote{The $\dashrightarrow$ denotes that it is only defined on some neighbourhood of $0$}, such that for all W-convex $\symcal{U}$, $\Flux_{\symcal{U}} ∘ v_{L_0} = \id$.
\end{definition}

\begin{lemma}
  If $v_{L_0}, v_{L_0}'$ are two versal deformations. For $α ∈ H^1(L_0;ℝ)$ close enough to $0$, $v_{L_0}(α) ≅ v_{L_0}'(α)$.
\end{lemma}
\begin{proof}
Let $(\symcal{U},φ)$ be a W-convex pair, and $v_{L_0}(α) = φ(Γ_β), v_{L_0}'(α) = φ(Γ_γ)$. Now by \cref{thm:graph_flux} $[γ] = \Flux_{\symcal{U}}(φ(Γ_β)) = α = \Flux_{\symcal{U}}(φ(Γ_β)) = [β]$, which proves the claim by \cref{thm:graph_hamiltonian_isotopic}.
\end{proof}
...
Let now $μ\colon M → B$ be a surjective regular Lagrangian fibration with simply connected base and moment map, and $L_0 = μ^{-1}(b_0)$.
Since $μ$ is a moment map, we also get that $M$ is symplectomorphic to $B × T^n$ with action angle coordinates $(b_i,θ_i)$, the elements $[d θ_i] ∈ H^1(μ^{-1}(b);ℝ)$ forming a basis. This gives an identification of $i\colon H^1(L_0;ℝ) → ℝ^n$.
Let $\symcal{L}_μ = μ^{-1}(B) ⊂ \symcal{L}_{L_0}$. Restricting to Lagrangian isotopies contained in $\symcal{L}_μ$, the flux map is well defined for all $μ^{-1}(b)$.
...
\begin{proof}
  Since $M = B × T^n = T^* T^n$, $ω = d λ$ where $λ = b_i d θ_i$ is the canonical 1-form.
  For $L = μ^{-1}(b)$ and $[a] ∈ H_1(L_0;ℝ)$ let $C_{[a]}$ a cylinder as in the definition of the flux map. Then
  \[⟨\Flux(L),[a]⟩ = ∫_{C_{[a]}} \\d λ = ∫_{φ ∘ a} λ - ∫_a λ = ∫_a φ^* λ \]
\end{proof}
...
\begin{theorem}
  ...
  where we're identifying $H^1(T_{kpq};ℝ)$ with $ℝ^2$ by the choosing the basis $(d θ_1, d θ_2)$ given by the action angle coordinates given by $μ$.
