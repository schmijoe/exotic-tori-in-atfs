...
By \todo{cite flux zeugs}, the flux map determines $τ$ locally up to a $\GL_2(ℤ)$ transform.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \begin{tikzpicture}[scale=.6]
      ...
      \draw[thick] (0,4) -- (0,-1);
    \end{tikzpicture}
    \caption{$\symbf{H}(X)$ with the focus-focus fibres marked by crosses.}
  \end{subfigure}%
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \begin{tikzpicture}[scale=.6]
      ...
      \draw[thick] (0,4) -- (0,-1);
    \end{tikzpicture}
    \caption{$μ(X)$ with the critical values marked by crosses, and the image of $ℓ$ by a dotted line.}
  \end{subfigure}
...
\begin{proof}
  ...
  $B_{dpq}$ deformation retracts to $\symbf{H}^{-1}(ℓ)$ where $ℓ = [0,b_2]$: We first vertically retract the space onto the preimage under $\symbf{H}$ of the ray $[0,∞) × \{0\}$ (for example by flowing along the gradient of the Morse-Bot function $|\symbf{H}_2|^2$). For all $x ∈ B_{dpq}$ with $\symbf{H}_1(x)>b_2$, we can flow along the gradient of the Morse function $\symbf{H}_1$ until they are in $\symbf{H}^{-1}(ℓ)$.
...
Behold the long exact sequence of homology for the pair $(S,∂ \hat{T})$:
...
Let $L_{b_0} = \pi^{-1}(b_0) \subset X$ be an Arnold--Liouville torus and let $\mathcal{U} \subset \mathcal{L}$ be a W-convex neighbourhood of $L_{b_0}$. Choose $V \subset B_0$ small enough so that $L_b \in \mathcal{U}$ for every $b \in V$. Then we can define the map $\phi_V \colon V \rightarrow H^1(L_{b_0};\mathbb{R})$ by $b \mapsto \Flux_{\mathcal{U}}(L_b)$. For a small enough $V$, this is a diffeomorphism onto its image. Now define
\[
    \label{eq:ltfVD}
    v_{L_b} \colon H^1(L_b;\mathbb{R}) \dashrightarrow \mathcal{L}, \quad
    v_{L_b}(a) = L_{\phi_V^{-1}(a)}.
\]
By definition we have $\Flux_{\mathcal{U}} \circ v_{L_b} = \id$, and thus by \cref{rem:lazy_vd}, we have proved the following. 

\begin{proposition}
    The map \eqref{eq:ltfVD} is a versal deformation. 
\end{proposition}

Any identification $H_1(L_{b_0};\mathbb{Z}) \cong \mathbb{Z}^n$ induces a dual identification $H^1(L_{b_0};\mathbb{R}) \cong \mathbb{R}^n$ which is unique up to transformations in the group of integral affine transformations in $\GL(n;\mathbb{Z}) \ltimes \mathbb{R}^n$. Equipped with charts of the form $\phi_V$, the set $B_0$ inherits the structure of an integral affine manifold. Furthermore, every map $\phi_V \circ \mathbf{H} \colon \pi^{-1}(V) \rightarrow \mathbb{R}^n$ yields action coordinates, see \cite[Lemma 2.15]{evans2021atfs}. Equivalently $\phi_V \circ \mathbf{H}$ is the moment map of a Hamiltonian $T^n$-action. Using a $T^n$-equivariant Weinstein chart of the orbit $L_{b_0}$, one can check directly that any moment map of a Hamiltonian $T^n$-action yields a canonical versal deformation by taking its fibres.  
