...
\begin{proof}
  ...
  Let $\hat{J}_X$ be a geometrically bounded almost complex structure on $X$, and $J_0$ the almost complex structure on $π^{-1}(⋃_{x ∈ V'} A_0(x))$ obtained by identifying $π^{-1}(⋃_{x ∈ V'} A_0(x))$ with $μ_0^{-1}(⋃_{x ∈ V'} A_0(x))$.
  Let $K ⊂ X$ be some compact set with its interior containing $π^{-1}(⋃_{x ∈ V'} A_0(x))$, and $J$ a geometrically bounded almost complex structure on $X$ with $J = φ_* J_0$ on $π^{-1}(⋃_{x ∈ V'} A_0(x))$ and $J = \hat{J}_X$ on $X ∖ K$.
  ...
  Since unlike $B_{dpq}$, $X$ might contain $J$-spheres, in order to get the desired result of \cref{thm:chekanov} we additionally require that $x_1 < σ_S(X,J)$, which is done by further restricting $V$.
\end{proof}
...
An ATF-corner $(x,dv)$ is \textbf{isolated} if $Δ$ contains no other ATF-corner at $x$.
\end{definition}

\begin{definition}
  \label{def:mutation_at_atf_corner}
  The \textbf{mutation at $(x,dv)$} is given by performing a change of branch cut at all nodes lying on the branch cut $v$ as in \cref{thm:change_branchcut} writing $\overline{S}_{(x,dv)}$ for the associated piecewise linear map, followed by a nodal slide to make the newly created branch cut negligibly small.
\end{definition}
