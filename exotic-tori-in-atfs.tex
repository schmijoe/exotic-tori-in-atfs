...
\]
by identifying $\symcal{V}$ with the copy $\symcal{V_0} ⊂ \tilde{\mathcal{L}}$ containing the constant loop at $L_0$.
...
\begin{remark}
  \label{rem:lazy_vd}
  In fact it is only required that the condition in \cref{def:versal_deformation} is satisfied for one simply connected neighbourhood $\symcal{U} ⊂ \symcal{L}$:
