...
The branch cut is still contained in the eigenline, but switches sides. 

\begin{proposition}
    \label{thm:change_branchcut}
    Let $\Delta \subset \mathbb{R}^2$ be an almost toric base diagram containing a node $n \in \Delta$.
Up to translation, we assume that $n = (0,0) \in \mathbb{R}^2$.
Furthermore, let $v \in \mathbb{Z}^2$ be the primitive vector determining the eigenline of the node $n$ pointing from the intersection of the branch cut with $∂Δ$ to $n$.
...
Indeed, we will see in \cref{sec:homology} that there is a set of generators of $H_2(B_{dpq})$ represented by Lagrangian spheres, meaning that $[\omega] = 0 \in H^2(B_{dpq};\mathbb{R})$.
...
$H_2(S,∂ \hat{T})$ is generated by the disks $D_0,…,D_d$, where the boundary of $D_0$ represents the unique class that retracts onto a point on the core circle in $S$.
These disks can be seen schematically in \cref{fig:homology_generating_discs}, where the disc $D_0$ intersecting the toric boundary collapses the $(0,1)$ cycle in the toric fibre $T(x)$ and the discs intersecting the critical points collapse the $(-q,p)$ cycle.
...
\end{lemma}
\begin{proof}
  Suppose that $\xi \in H_1(L_0)$ and let $γ:S¹ \to L_0$ be a parametrisation of $\xi$.
...
In examples, we work with identifications $H^1(L';\mathbb{R}) \cong H^1(L;\mathbb{R})\cong \mathbb{R}^n$ induced by a choice of basis $H_1(L) \cong H_1(L') \cong \mathbb{Z}^n$, in which case the isomorphism \eqref{eq:phi_isomorphism} corresponds to an isomorphism $\phi \in \operatorname{GL}(n;\mathbb{Z})$.
...
\begin{proposition}
  ...
  Then there are infinitely many $d_i,p_i,q_i$ such that there is a partial $B_{d_i,p_i,q_i}$-embedding into $M$.
\end{proposition}
...
\begin{definition}
  ...
  An \textbf{ATF-corner} of $Δ$ is a pair $(x,dv)$ with $x ∈ ∂Δ$ a corner, $v ∈ ℤ²$ the primitive direction of a branch cut intersecting $∂Δ$ at $x$ pointing into $Δ$, and $d$ the number of nodes on that branch cut.
An ATF-corner $(x,dv)$ is \textbf{isolated} if it is the only ATF-corner at $x$.
\end{definition}

\begin{definition}
  \label{def:mutation_at_atf_corner}
  The \textbf{mutation at $(x,dv)$} is given by performing a change of branch cut at all nodes lying on the branch cut $v$ as in \cref{thm:change_branchcut} writing $\overline{S}_{(x,dv)}$ for the associated piecewise linear map, followed by nodal slides to make the newly created branch cut negligibly small.
\end{definition}
...
\begin{definition}
  ...
  A non-isolated ATF-corner $(x,dv)$ can be isolated by mutating at all other ATF-corners at $x$.
It is then assigned the type of that isolated ATF-corner.
\end{definition}
...
\end{figure}
  ...
  Let $ℓ_n = ℓ(e_{n}) - ℓ(e_0)$, and $ℓ_∞$ the limit $ℓ_n$ for $n → ∞$.
...
Since $g_{n_i}(w_i) > g_n(u_{k_n}^{n+1})$ for all $n<n_i-1$, $w_i$ lies in the fixed half plane  for all mutations $\overline{S}_{u_{k_n}}^n$ with $n< n_i-1$ by the convexity of the $Δ_n$.
  ...
  $v_∞$ is not parallel to $x_f$ (it would contradict the convergence of $w_i$), so $p_i = \det(x_f, -v_i) → ∞$, so there is an ATF-corner of type $d_i,p_i,q_i$ at $w_i$ in $Δ_{n_i}$, with $p_i → ∞$.

  In any case, using \cref{thm:toric_embeddings} we get partial $B_{d_i,p_i,q_i}$-embedding for all $i ∈ ℤ$.
\end{proof}
