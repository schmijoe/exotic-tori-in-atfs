...
This paper is motivated by the symplectic classification question of Lagrangian submanifolds: For $L,L' \subset (X,\omega)$ Lagrangian submanifolds of a symplectic manifold, can $L$ be mapped to $L'$ by a symplectomorphism of $X$? We say that $L,L'$ are \emph{equivalent} and write $L \cong L'$ if such a symplectomorphism exists. 

An obvious necessary condition for $L$ and $L'$ to be equivalent is that they are diffeomorphic as manifolds. In this paper, we only consider Lagrangian tori. They play a special role in symplectic geometry, since they are \emph{everywhere}. Indeed, Darboux theorem states that every symplectic manifold locally looks like a copy of the standard symplectic vector space and the latter contains arbitrarily Lagrangian tori. Furthermore, Lagrangian tori naturally appear in classical mechanics, and more specifically the study of completely integrable systems. In fact, in this paper we heavily rely on certain completely integrable systems (so-called almost toric fibrations) as an auxiliary tool to study the symplectic topology of their fibres.

Recall that every Lagrangian submanifold $L \subset X$ has two characteristic classes; the \emph{area class} $a_L \colon H_2(X,L) \rightarrow \mathbb{R}$ and the \emph{Maslov class} $m_L \colon H_2(X,L) \rightarrow \mathbb{Z}$, which are preserved by symplectomorphisms in the sense that if there is $\phi \in \operatorname{Symp}(X,\omega)$ with $\phi(L) = L'$, then 
\begin{equation}
    \label{eq:softcond}
    a_{L} = a_{L'} \circ \Phi, \quad
    m_{L} = m_{L'} \circ \Phi, 
\end{equation}
where $\Phi = (\phi\vert_L)_* \colon H_2(X,L) \rightarrow H_2(X,L')$. Therefore, the existence of some $\Phi$ such that \eqref{eq:softcond} holds is a necessary condition for $L$ and $L'$ to be equivalent. If such a map $\Phi$ exists, we say that $L$ and $L'$ have the \emph{same classical invariants} and write $L \sim L'$.

Finding Lagrangian tori which have the same classical invariants but are \emph{not} equivalent is a classical question in symplectic topology, which was initiated by \cite{Che96} and \cite{EliPol97} and has attracted much interest since.  see  (this is a non-exhaustive list)\cite{Aur15, brendel2020real, CheSch10, FOOO12, GalMik22, SheTonVia19, Via17, Via16}. Such Lagrangian tori are sometimes called \emph{exotic} or \emph{knotted}. 

The first-named author has recently proved that in dimension six and higher, the existenc of exotic tori is generic and a purely local occurence. 
...
The tori in \cref{thm:Bre23} are constructed by embedding exotic tori in $\mathbb{R}^{2n}$ via Darboux charts into $X$. The existence of infinitely many exotic Lagrangian tori in $\mathbb{R}^6$ is due to \cite{Aur15}. Note that the tori in \cite{Aur15} are monotone and are distinguished by the count of Maslov index two $J$-holomorphic disks. For non-monotone tori, this count is not well-defined because of bubbling. Since monotonicity is lost after embedding the tori into $X$, this method of distinguishing the corresponding Lagrangian tori in $X$ fails. In \cite{brendel2023local} and this paper, we instead rely on the so-called \emph{displacement energy germ} $[e]_L \colon H^1(L;\mathbb{R}) \rightarrow \mathbb{R} \cup \{+\infty\}$ to distinguish Lagrangian tori. This invariant is based on the approach of versal deformations, introduced in \cite{Che96}. Roughly speaking, $[e]_L$ measures how the displacement energy behaves as a function on tori in a neighbourhood of $L$, see \cref{sec:versal_deformations} for details. For our purposes, the two following properties of the displacement energy germ are crucial: 
\begin{enumerate}
    \item The displacement energy germ $[e]_L$ is well-defined, even if $L$ is not monotone, 
    \item Let $\varphi \colon \mathbb{R}^{2n} \supset U \rightarrow X$ be a Darboux chart. Then for small enough tori $L \subset U$, the displacement energy germ is preserved by the Darboux embedding, 
    \begin{equation}
        \label{eq:locality}
        [e]_L \circ (\varphi\vert_L)^* = [e]_{\varphi(L)}.
    \end{equation}
\end{enumerate}
We call the latter property the \emph{locality of the displacement energy germ}. Note that this property is not trivial. Indeed, the germ $[e]_L$ is derived from the displacement energy of a family of tori in $\mathbb{R}^{2n}$, whereas $[e]_{\varphi(L)}$ is derived from the displacement energy of tori in $X$.

\subsection{Dimension four}

In this paper, we consider the case of $\operatorname{dim} X = 4$. The main difference with the case $\operatorname{dim} X \geqslant 6$ is the lack of exotic tori in $\mathbb{R}^4$: Besides product tori and the Chekanov torus \cite{Che96}, there are no other known tori in $\mathbb{R}^4$. In fact, a parial classification result by \cite{Riz19} suggests that there are no other Lagrangian tori in $\mathbb{R}^4$, although this is still open. This means that the approach of embedding Lagrangian tori via Darboux charts is inefficient in dimension four. Even embedding the Chekanov torus into some four-dimensional symplectic manifold $X$ by a Darboux chart does not yield a torus which is exotic in a robust sense:
...
Thus the approach we use and the flavour of results we obtain are markedly different from \cite{brendel2023local}. Instead of constructing exotic tori in a Darboux chart, we construct exotic tori in certain, more complicated symplectic domains.



$(B_{dpq}, \omega)$ and prove that the tori remain exotic under certain symplectic embeddings $\varphi \colon B_{dpq} \supset U \rightarrow X$ into a geometrically bounded symplectic manifold $X$. The spaces $B_{dpq}$ naturally appear as Milnor fibres of smoothings of certain cyclic quotient singularities. 

Let us illustrate what happens in the simplest, non-trivial case. For $d = 2, p = q = 1$, we have $(B_{2,1,1}, \omega) \cong (T^*S^2, -d\lambda_{\operatorname{can}})$. For every $a > 0$, the space $T^*S^2$ contains a Clifford torus $T_{ \operatorname{Cl}}(a)$, a Chekanov torus $T_{\operatorname{Ch}}(a)$ and a so-called Polterovich torus $T_{\operatorname{Pol}}(a)$, all of which are monotone of monotonicity constant $a$. These tori satisfy $T_{ \operatorname{Cl}}(a) \sim T_{\operatorname{Ch}}(a) \sim T_{\operatorname{Pol}}(a)$, but they are two-by-two inequivalent up to symplectomorphisms of $T^*S^2$. Now let $\varphi \colon T^*S^2 \supset U \rightarrow X$ be a symplectic embedding of a neighbourhood $U$ of the zero-section. We prove (see \cref{thm:...}) that for small enough $a > 0$, the images $\varphi(T_{ \operatorname{Cl}}(a)), \varphi(T_{ \operatorname{Ch}}(a)), \varphi(T_{ \operatorname{Pol}}(a))$, though they have the same classical invariants, are still two-by-two inequivalent by symplectomorphisms of $X$. In particular, using Weinstein's neighbourhood theorem, we can prove the following.

\begin{corollary}
    Let $N \subset (X,\omega)$ be a Lagrangian sphere in a geometrically bounded symplectic four-manifold. Then every tubular neighbourhood of $N$ contains three Lagrangian tori $L_{ \operatorname{Cl}}, L_{ \operatorname{Ch}}, L_{\operatorname{Pol}}$ which have the same classical invariants but are two-by-two inequivalent up to symplectomorphisms of $X$. 
\end{corollary}
...
Which partial $B_{dpq}$-embeddings into $X$ are there? Note that \cite{EvaSmi18} proved that ... Thus this gives an additional motivation to study such embeddings. From the viewpoint of algebraic geometry, they come from degenerations of $X$ to singular varieties having certain cyclic quotient singularities.

Contrary to the case treated in \cite{brendel2023local}, which relies on Darboux embeddings, the existence of partial $B_{dpq}$-embeddings into $X$ is not automatic. In fact, \cite{EvaSmi18} proved some stringent conditions


One of the key differences between the purely local exotic Lagrangian tori constructed in \cite{brendel2023local} and the ones constructed in this paper is the fact 
...
In Evans' and Urzúa's paper \cite{EvaUrz21} they give a construction that yields four dimensional symplectic surfaces $(X,\omega)$, equipped with specific, non-standard symplectic forms, that admit unbounded sequences of partial $B_{pq}$ embeddings, or equivalently sequences of Lagrangian embeddings of pinwheels $L_{pq}$, in which $p$ goes to infinity. In their construction Mori's theory of flips is realized as a series of mutations and deformations of the symplectic form, that one can find in certain configurations of almost toric fibrations. In light of the main result obtained in this paper, namely \cref{thm:main_embedded}, their results yield the following. 

\begin{theorem}
    If $(X,\omega)$ is one of the symplectic manifolds discussed in \cite{EvaUrz21}, i.e. either a specific quintic surface or a Godeaux surface, in either case containing certain Mori sequences, then $(X,\omega)$ contains an infinite sequence of distinct exotic Lagrangian tori.
    \begin{proof}
        This follows immediately from the constructions in \cite{EvaUrz21} and the results obtained in \cref{thm:main_embedded}.
    \end{proof}
\end{theorem}
...
\begin{remark}
  ...
  We can calculate the type of an ATF-corner $(x,dv)$ as follows: Take $u_1,u_2$ to be a $ℤ$-basis of $ℤ²$ with $u_1$ pointing from $x$ along $∂Δ$ in the counter-clockwise direction.
...
In the following section we give a self contained introduction to versal deformations.
...
For the remainder of this section we want to fix a compact Lagrangian $L_0 ⊂ (X,ω)$.
...
\begin{corollary}
  ...
  The subspace $\symcal{L}_Γ$ contains a neighbourhood of $L_0 \in \symcal{L}_{L_0}$, by the definition of the $\symcal{C}^1$-topology, and therefore $\Flux$ yields a map
...
\begin{proposition}
  ...
  By this we mean that any $L_1, L_2 \in \symcal{U}$ are Hamiltonian isotopic by a Hamiltonian isotopy through $\symcal{U}$ if and only if $\Flux_{\symcal{U}}(L_1) = \Flux_{\symcal{U}}(L_2)$.
\end{proposition}
\begin{proof}
...
Then 
    ...
    The last equality follows from Definition \eqref{eq:localflux} of $\operatorname{Flux}_{\symcal{U}}$ and the fact that we can homotope any given Lagrangian isotopy in $\symcal{U}$ with endpoints $L$ and $\mu^{-1}(x_0 + a)$ to $\Lambda$ by the fact that $\symcal{U}$ is simply connected. 
...
\begin{corollary}
  ...
  Suppose $A^+, A^- ⊂ M$, with both $A^±$ non-empty, $Σ$ is a connected Riemann surface with $∂Σ = ∂Σ^+ ⊔ ∂Σ^-$, with both $∂Σ^±$ non-empty and $u\colon Σ → M$ a $J$-holomorphic curve, with $u(∂Σ^±) ⊂ A^±$ respectively.
...
\begin{proof}
  ...
  We want to place pairwise disjoint balls onto $γ$, ther $N$
...
\begin{proof}
  Let $V \subset \Delta_{dpq}$ be a subset such that $e(φ(T(x))) ≤ x_1$ for all $x ∈ V$, as given by \cref{thm:upper_bound_embedded}.
  Proceed in a similar way as in \cref{thm:displacement_energy}: By restricting $V$, we may assume that $A_0(x) = \{y ∈ \im μ_0 \mid d_{μ_0}(x,y) ≤ δ \}$ as defined in the proof of \cref{thm:displacement_energy} such that $\pi^{-1}(A_0(x))$ is contained in $U$ for all $x ∈ V$.
  Assume additionally that $φ(π^{-1}(\symcal{A}_0))$, where $\symcal{A}_0 = ⋃_{x ∈ V} A_0(x)$, is contained in the interior of some compact set $K \subset X$. This can be achieved by further restricting $V$.
  ...
  Let $J$ a geometrically bounded almost complex structure on $X$ with $J = φ_* J_0$ on $φ(π^{-1}(\symcal{A}_0))$ and $J = J_X$ on $X ∖ K$. See \cref{rem:J_compactly_perturbed}.
  For any fixed $x ∈ V \setminus R$, we conclude as in \cref{thm:displacement_energy} that any $J$-disk $u \colon (D,∂D) → (X,φ(T(x)))$ has at least area $x_1$. 

  Unlike $B_{dpq}$, the space $X$ might contain $J$-spheres. In order to deduce the claim from \cref{thm:chekanov}, we additionally require that $x_1 < σ_S(X,J)$, which is done by further restricting $V$. Since $e(φ(T(x))) ≤ x_1$, we conclude from \cref{thm:chekanov} that there is a $J$-disk $u$ of area $x_1$ and hence the claim follows.
\end{proof}
...
\begin{remark}
    Let us briefly describe what the algorithm in the proof of \cref{thm:toric_embeddings} yields for the case of $X = \mathbb{C}P^2$. Vianna \cite{Via16} proved that the set of ATF-base diagrams of $\mathbb{C}P^2$ obtained from the toric Delzant polytope by mutations is (up to nodal slides) in bijection with so-called Markov triples, i.e.\ solutions to the Markov equation.
For more details on this, we refer to \cite[Section 8.4, Appendix I]{evans2021atfs}. ....
\end{remark}
...
\emph{Step 1 : }
Start by performing a nodal trade at every toric vertex of $\Delta$. Now we define a sequence of ATF-mutations of $\Delta$.
Fix an edge $e \subset \Delta$, which we call the \emph{distinguished edge}.
For simplicity, assume that $e \subset O_x = \{(x,0)\}$ and that $\Delta$ is contained in the upper half plane $\{y \geqslant 0\}$.

Let $\{\Delta_n\}_{n\in \mathbb{N}}$ be the sequence of ATF-base diagrams of $X$ obtained by successively mutating at the left vertex of $e$.
More precisely, set $\Delta_0 = \Delta$ and let $z_0$ be the ATF-corner associated to the left vertex of the distinguished edge $e_0 = e$ in case that vertex is simple.
In case that vertex is not simple, let $z_0$ be its ATF-corner with the smalles angle between branch cut an $e_0$.
Now define $\Delta_1 = \overline{S}_{z_0} \Delta_0$.
Note that $\Delta_1$ has a unique edge $e_1$ contained in $O_x$, satisfying $e_0 \subset e_1$.
This inclusion is strict if and only if the vertex at which we mutated was simple.
We call $e_1 \subset \Delta_1$ the \emph{distinguished edge}, as well.
Let $z_1$ be the ATF-corner on the left vertex of $e_1$, defined similarly as $z_0 \in \Delta_0$. Iterating this construction, we obtain a sequence of ATF-base diagrams $\{\Delta_n\}_{n \in \mathbb{N}}$, distinguished edges $e_n \subset \Delta_n$ and adjacent ATF-corners $z_n \in e_n \subset \Delta_n$, such that $\Delta_{n+1} = \overline{S}_{z_n} \Delta_n$. See \cref{fig:stretch_iteration}.

The distinguished edges are nested, meaning that $e_n \subset e_{n+1}$, where the inclusion is strict whenever the left vertex of $e_n$ is simple.
Note that after a finite number of mutations at a non-simple vertex, it becomes simple and thus the sequence $\{e_n\}_{n \in \mathbb N}$ contains a subsequence of edges which becomes strictly longer.
\smallskip

\emph{Step 2 : }
Let $u^0_1, \ldots , u^0_N$ be the ATF-corners of $\Delta_0$. The set $\{1,\ldots,N\}$ will serve as labels for the ATF-corners throughout the mutations.
The labelling for $n=0$ is arbitrary and the labels for $n > 0$ are recursively assigned as follows:
For every $k \in \{1,\ldots,N\}$ such that $u^n_k \neq z_n$, i.e.\ for all vertices at which we do not mutate, set $u_k^{n+1} = \overline{S}_{z_n} u_k^n$. For $k$ with $u_k^n = z_n$, i.e. the vertex at which we mutate, let $u_k^{n+1}$ be the vertex that is newly created by the mutation at $z_n$. 
In other words, every ATF-corner surviving mutation keeps its label and the ATF-corner at which we mutate cedes its label to the newly created ATF-corner.
See \cref{fig:stretch_iteration} for an illustration in which the labels are represented by colours.
Although the total number of ATF-nodes is constant throughout mutations, note that the number of ATF-corners may decrease if two opposite corner have the same eigenline\footnote{Consider for example any mutation of the square (the toric base of a monotone $S^2 \times S^2$) in the plane.}. This means that we may have $u_i^n = u_j^n$ for some $n$ and $i \neq j$, meaning that an ATF-corner carries a double label.
Since this occurs at most a finite number of times, we can assume it does not occur at all by choosing some $n_0$ large enough such that $Δ_{n_0}$ has the minimum number of ATF-corners in $\{Δ_n\}_n$ and setting $\Delta_0 ≔ \Delta_{n_0}$.

Let 
\begin{equation}
    \label{eq:ln}
    l_n = \{ k \in \{1,\ldots,N\} \, \vert \, z_n = u_k^n\} 
\end{equation}
be the label of the ATF-corner at which the $n$-th mutation is performed.
There are two types of labels - those appearing finitely many times in the sequence $\{l_n\}_{n \in \mathbb{N}}$ and those appearing infinitely many times.
Up to relabelling, we can assume that the labels appearing infinitely many times are given by\footnote{Based on computer simulations, we suspect that $N'=2$. This means that our sequence of mutations $\{\Delta_n\}_{n \in \mathbb{N}}$ stabilizes to a sequence of mutations considered in \cite{EvaUrz21}.} $\{1,\ldots,N'\}$, for $N' < N$.

We assume without loss of generality that none of the labels appear only finitely many times:
If this is not the case, let $n_1 \in \mathbb{N}$ be large enough such that $l_n \in \{1, \ldots,N'\}$ for all $n \geqslant n_1$, i.e.\ none of the labels appearing finitely many times in $\{l_n\}_{n \in \mathbb{N}}$ appear after the $n_1$-th mutation.
Then set $Δ_0 ≔ Δ_{n_1}$.
\smallskip

\emph{Step 3 : } 
We parameterize points on the boundary of the ATF-base diagrams $\Delta_n$ by their integral affine arc length as follows. 
Note that the right vertex $v_r$ of the distinguished edges $e_n$ coincides for all $n$, since it lies in the half plane in which $\overline{S}_{z_n} = \id$ and hence $v_r \in \partial \Delta_n$ for all $n$.
Let $P > 0$ be the integral affine perimeter of $\Delta_n$, i.e.\ the sum of integral affine lengths of its edges.
Note that it is preserved by mutations and thus independent of $n$. Let 
\[
    g_n \colon \partial \Delta_n \rightarrow [0,P)
\]
be the function such that $g_n(x)$ measures the integral affine length of the clockwise arc in $\partial \Delta_n$ between $v_r$ and $x \in \partial \Delta_n$.
The function $g_n$ is a bijection and continuous away from $v_r$.
Note that
\begin{equation}
    \label{eq:gn_invariance}
    g_{n+1}(\overline{S}_{z_n}(x)) = g_n(x), \quad
    \forall x \in \partial \Delta_n.
\end{equation}
Define
\begin{equation}
    \label{eq:alphabeta}
    \alpha_n
    = \min_{1 \leqslant i \leqslant N'} g_n(u_i^n), \quad
    \beta_n
    = \max_{1 \leqslant i \leqslant N'} g_n(u_i^n).
\end{equation}
This are the sequences of minimal/maximal integral affine distances of $v_r$ to the vertices carrying labels which appear infinitely many times in the sequence \eqref{eq:ln} of labels at which mutations are performed. 

\emph{Claim : } Both sequences $\{\alpha_n\}_{n \in \mathbb{N}}$ and $\{\beta_n\}_{n \in \mathbb{N}}$ are monotonically increasing and contain strictly increasing subsequences. 

Note that $\alpha_n$ is equal to the integral affine length of the distinguished edge $e_n$. Since $e_n \subset e_{n+1}$, the sequence $\{\alpha_n\}_n$ is monotonically increasing.
The inclusion $e_n \subset e_{n+1}$ is strict if and only if the ATF-corner at which the mutation was performed was isolated.
After a finite number of mutations, we mutate at an isolated ATF-corner, so the sequence $\{\alpha_n\}_n$ has a strictly increasing subsequence.

The sequence $\{\beta_n \}_n$ is monotonically increasing since we have $g_{n+1}(u_k^{n+1}) \geqslant g_n (u_k^n)$:
If $u_k^n \neq z_n$, then $g_{n+1}(u_k^{n+1}) = g_n (u_k^n)$ by \eqref{eq:gn_invariance}, and if $u_k^n = z_n$ we have $g_{n+1}(u_k^{n+1}) > g_n (u_k^n)$.

Indeed, this follows from \eqref{eq:gn_invariance} together with the fact that, at the $n$-th step, we mutate at the vertex realizing the minimum $β_n$, meaning that, when mutating, the minimal value in $\{g_n(u_i^n)\}_{1 \leqslant i \leqslant N'}$ is eliminated and all others preserved, see also \cref{fig:stretch_iteration}.
Therefore, the sequence $\{\beta_n \}$ has a strictly increasing subsequence.
If it did not, then there would be an ATF-corner $v_0$ in all $\Delta_n'$ for $n \geqslant n_2$ for some large $n_2$ at which no more mutations are performed.
However, this would mean that the label of $n_2$ would appear only a finite number of times in the sequence \eqref{eq:ln}. This is a contradiction with the definition of $\Delta_n'$ and hence we have proved the claim.

In particular, since both sequences are bounded, this means that they converge to some real numbers $\alpha_{\infty}$ and $\beta_{\infty}$. \smallskip

\emph{Step 4 : }
Let $\{\beta_{n_k}\}_{k \in \mathbb{N}}$ be a strictly increasing subsequence of the sequence $\{\beta_n\}$ defined in \eqref{eq:alphabeta}. Let $w_k \in \Delta_{n_k}$ be the ATF-corner at which the corresponding mutation has been performed and $w_k' \in \Delta_{n_k}$ the ATF-corner that has been created by the mutation. By construction, the corner $w_k'$ realizes the maximal integral affine distance along the boundary to $v_r$, i.e.\ $g_{n_k}(w_k') = \beta_{n_k}$.

\emph{Claim : } The sequences $\{w_k\}$ and $\{w_k'\}$ converge to points $w_{\infty} \in \mathbb{R}^2$ and $w_{\infty}' \in \mathbb{R}^2$, respectively. 

Let $v_r = (x_0,0)$ be the right vertex adjacent to the distinguished edges $e_{n_k}'$. Then $w_k = (x_0 - \alpha_{n_k-1},0)$ and since $\{\alpha_n\}$ converges, so does $w_k$. Note that the points $w_k$ are contained in the boundary of the initial ATF-base diagram $\Delta_0'$. Indeed, since $w_k'$ satisfies $g_{n_k}(w_k') = \beta_{n_k}$ and the sequence $\{\beta_n\}$ is monotone increasing, all of the mutations from $\Delta_0$ to $\Delta_{n_k}$ act by the identity on $w_k'$. Since furthermore $\{\beta_n\}$ converges, we find, by \eqref{eq:gn_invariance}, that $w_k'$ converges to the point $w_{\infty}'$ satisfying $g_0(w_{\infty}) = \beta_{\infty}$. This proves the claim.

Furthermore, note that all $w_k'$ for large enough $k$ and $w_{\infty}$ are contained in a fixed edge $f \subset \Delta_0$. Let $x_f$ be the primitive vector defining $f$, pointing in the counter-clockwise direction. Similarly, let $x_e = (1,0)$ be the primitive vector defining the distinguished edges.  
Let $v_k$ be the primitive vector pointing along $w_k'-w_k$ and define $v_∞ = w_∞' - w_∞$.
By definition we have $v_{k+1} = v_k - λx_f + μx_e$ for some $λ,μ>0$ and $k$ big enough.
Furthermore, $\det(v_k,x_f)>0$ by convexity of the ATF-base diagram $Δ_n$ and also $\det(v_k,x_e)<0$.
So $\det(v_{k+1},v_k) > 0$.
Let $π \colon ℝ² ∖\{0\} → S¹$ be the radial projection onto the unit circle.
 Since $\det(v_k,x_e)<0$, the points $\pi(v_k)$ are contained in a half-circle, and thus they converge strictly monotonically to $π(v_∞)$.
Since $v_k$ is primitive, the Euclidean length of the $v_k$'s satisfies $\norm{v_k} → ∞$. The vector $v_∞$ is not parallel to $x_f$, since this would contradict the convergence of $w_k$, so $p_k = \det(x_f, -v_k) → ∞$. Hence, $w_k$ is an ATF-corner of type $(d_k,p_k,q_k)$ for some $d_k,q_k$ in $Δ_{n_k}$, with $p_k → ∞$.

Using \cref{thm:toric_embeddings} we get a partial $B_{d_k,p_k,q_k}$-embedding for all $k ∈ \mathbb{N}$.
\end{proof}
...
This is the edge that will be elongated by iterated mutations at an adjacent vertex. We may assume $e_0 \subset $
...
Let $Δ_{n+1} = \overline{S}_{u_{k_n}^n} Δ_n$ be the result of mutating $Δ_n$ at the ATF-corner $u_{k_n}^n$ next to $e_n$ in the clockwise direction (if $e_n$ has multiple non-isolated ATF-corners there, we pick $u_{k_n}^n = (x,v)$ for which $v$ forms the smallest angle with $e_n$).
...
\end{figure}
  ...
  Note that $e_n$ lies in the fixed half plane of $\overline{S}_{u_{k_n}^n}$, so we pick the edge $e_{n+1}$ of $Δ_{n+1}$ such that $e_n ⊂ e_{n+1}$.
  ...
  The sequence $k_n ∈ \{1,…,n_c\}$ contains some elements of $\{1,…,n_c\}$ infinitely many times.
  We may assume without loss of generality that all the elements which appear, appear infinitely many times (If this is not the case, we can choose a $N$ such that this holds for $k_n$ with $n≥N$, then setting $Δ_0=Δ_N$.), and that they are $\{1,…,n_c'≤n_c\}$ (this can be achieved by relabelling).
...
Note that we have $g_n(x) = g_{n+1}(\overline{S}_{u_{k_n}^n}x)$, since integral linear transformations preserve the affine length.
  ...
  This is the distance from $u_{k_0}^0$ to furthest away (in terms of $g_n(\cdot)$) ATF-corner appearing in infinitely many mutations.
We have $a_n ≤ a_{n+1}$, since $g_n(u_k^n) = g_{n+1}(u_k^{n+1})$ if $k ≠ k_n$ and $g_n(u_{k_n}^n) < g_{n+1}(u_{k_n}^{n+1})$.
Also $a_n > ℓ_n = g_n(u_{k_n}^n)$ and $a_n ≤ ℓ_m$ for some $m>n$. The latter claim follows from the fact that each $1 ≤ k ≤ n_c'$ must appear in $k_{m}$ for $m>n$ and thus there is a mutation at which the portion of $∂ \Delta_n$ of which $a_n$ measures the integral affine length is straightened out and absorbed into $e_{k_m}$.
  In particular $a_n → ℓ_∞$ and $a_n$ has a strictly monotone subsequence $a_{n_i}$ such that $a_{n_i} > a_{n_i - 1}$.
...
Since $g_{n_i}(w_i) > g_n(u_{k_n}^{n+1})$ for all $n<n_i-1$, $w_i$ lies in the fixed half plane  for all mutations $\overline{S}_{u_{k_n}}^n$ with $n< n_i-1$ by the convexity of the $Δ_n$. See \cref{fig:stretch_wi}.
