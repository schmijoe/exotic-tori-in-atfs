...
In a certain class of spaces $B_{dpq}$ (which natually appear in almost toric fibrations), we find $d+1$ families of monotone Lagrangian tori which are mutually distinct up to symplectomorphisms.
...
This paper is motivated by the symplectic classification question of Lagrangian submanifolds: For $L,L' \subset (X,\omega)$ Lagrangian submanifolds of a symplectic manifold, can $L$ be mapped to $L'$ by a symplectomorphism of $X\,$? We say that $L,L'$ are \textbf{equivalent} and write $L \cong L'$ if such a symplectomorphism exists.
...
Indeed, Darboux theorem states that every symplectic manifold locally looks like a copy of the standard symplectic vector space and the latter contains arbitrarily small Lagrangian tori.
Furthermore, Lagrangian tori naturally appear in classical mechanics, and more specifically the study of completely integrable systems.
...
\begin{equation}
    ...
    m_{L} = m_{L'} \circ \Phi, 
\end{equation}
where $\Phi = (\phi\vert_L)_* \colon H_2(X,L) \rightarrow H_2(X,L')$.
Therefore, the existence of some $\Phi$ such that \eqref{eq:softcond} holds is a necessary condition for $L$ and $L'$ to be equivalent.
If such a map $\Phi$ exists, we say that $L$ and $L'$ have the \emph{same classical invariants} and write $L \sim L'$.
Recall also that $L$ is called \textbf{monotone (with monotonicity constant $C > 0$)} if $a_L = C m_L$. 

A full classification of Lagrangian tori up to symplectomorphisms is out of reach in most ambient spaces of dimension greater than two.
Note that the classification is not even known for $\mathbb{R}^4$, although there is some evidence \cite{Riz19} that every Lagrangian torus in $\mathbb{R}^4$ is Hamiltonian isotopic to either a product or a Chekanov torus.
...
Lagrangian tori which are not equivalent to known ones (such as product tori in $\mathbb{R}^{2n}$) are sometimes called \emph{exotic} or \emph{knotted}. 

\subsection{Main results}
\label{sec:intro_main_results}

In this paper, we study exotic Lagrangian tori in dimension four.
We construct non-equivalent Lagrangian tori in certain model spaces $B_{dpq}$ and prove that, if there is a symplectic embedding $B_{dpq} \supset U \rightarrow X$ into a geometrically bounded $(X,\omega)$, then the images of small enough tori are non-equivalent with respect to symplectomorphisms of the new ambient space $X$.

More precisely, let $d,p,q \in \mathbb{N}$ be integers such that $d≥1$ and $p,q$ are coprime with $p>q≥0$.
For every such triple, there is a symplectic manifold $B_{dpq}$, which will serve as model space for us.
There are two complementary ways of viewing the spaces $B_{dpq}$.
...
In order to embed the exotic tori from \cref{thm:A} into a general symplectic manifold $(X,\omega)$, we consider certain symplectic embeddings of the form $\varphi \colon B_{dpq} \supset U \rightarrow X$, which we call \textbf{partial $B_{dpq}$-embeddings}, see \cref{def:partial_embedding}.
Every space $B_{dpq}$ contains a configuration consisting of 
\begin{enumerate}
...
From a symplectic point of view, this union can be viewed as Lagrangian skeleton of the Stein domain $B_{dpq}$ and from the algebraic geometry point of view it can be viewed as vanishing cycle of the degeneration of $B_{dpq}$ to the corresponding cyclic quotient singularity.
See \cite{Eva19} for a details on the latter point of view.

Roughly speaking, a partial $B_{dpq}$-embedding is a symplectic embedding of an open neighbourhood of that union of visible Lagrangians.
Our second main result distinguished the images of small enough tori $T^k_{pq}$ via partial $B_{dpq}$-embeddings into geometrically bounded symplectic manifolds. 
...
Then every tubular neighbourhood of $N$ contains three different types of tori, Chekanov, Clifford and Polterovich, which are not related by symplectomorphisms of $X$.
\end{corollary}
...
On the other hand, we prove the following, see \ref{thm:toric_embeddings} for the detailed statement.
...
\begin{maintheorem}
    \label{thm:C}
    Let $(X,\omega)$ be a compact four-dimensional symplectic manifold admitting an (almost) toric structure with a base diagram having finitely many nodes.
For every $i \in \mathbb{N}$, there is $a_{i,0} > 0$ such that for every $i \neq j$, we have
    \begin{equation}
...
One drawback of this result is the fact that the sizes of embeddings may become arbitrarily small along the sequence, meaning $a_{i,0} \rightarrow 0$. 

As was pointed out to us by Jonny Evans, we can apply \cref{thm:B} to certain surfaces of general type studied \cite{EvaUrz21}.
The key point in that paper is interpreting Mori's theory of flips in terms of almost toric base diagrams and finding a sequence of mutations (using only two vertices) yielding an infinite sequence of partial $B_{1,p_i,q_i}$-embeddings with $p_i \rightarrow \infty$.
...
\begin{corollary}
    ...
    Let $X$ be quintic surface or a simply connected Godeaux surface.
...
Vianna \cite{Via16,Via17} noticed that the regular fibres of almost toric fibrations are a rich source of exotic Lagrangian tori.
...
The pair $(p,q)$ determines the integral affine angle between the branch cut an the boundary of the almost toric base diagram.
This is discussed in detail in \ref{sec:modelspaces}.
...
The lower bound is given $J$-holomorphic curves via Chekanov's estimate (\cref{thm:chekanov}) together with an area estimate on $J$-holomorphic curves leaving the partial $B_{dpq}$-embedding.
...
In \cref{into:discussion}, we discuss the parallels and differences between the constructions in dimensions $\geqslant 6$ from \cite{brendel2023local} and our constructions in dimension $4$. 
...
Then every open set of $X$ contains infinitely many tori which have the same classical invariants, but are two-by-two inequivalent up to symplectomorphisms of $(X,\omega)$.
\end{theorem}
...
In fact, a partial classification result by \cite{Riz19} suggests that there are no other Lagrangian tori in $\mathbb{R}^4$, although this is still open.
...
In \cite{FOOO12}, the authors detect a continuum of non-displaceable tori and exoticity follows from non-displaceability in that case.
