...
Note that the intersection $\hat{S}_k = ⋃_{ρ ∈ ρ_p} π_3^{-1}([ρ a_k^{2/p},ρ a_{k+1}^{2/p}]) ∩ \symbf{H}_2^{-1}(0)$ forms a collection of $p$ Lagrangian\todo{why?} disks for $k=0$ and spheres for $0<k<d$, with the disks intersecting on their boundary $\{z_3=0,\symbf{H}_2=0\}$, see \cref{fig:conic_fibration} (c).
...
Let $L_0 ⊂ (X,ω)$ be a compact Lagrangian.
...
By $\symcal{L}_{L_0}$ (or simply $\symcal{L}$), denote the set of Lagrangians which are Lagrangian isotopic to $L_0$, equipped with the $\symcal{C}^1$-topology.
...
where we have identified $V$ with the copy $V_0 \subset \Tilde{\mathcal{L}}$ containing the constant loop. 
...
Then $L_0$ admits an embedding of a Weinstein-neighbourhood $φ: T^*L_0 \dashrightarrow X$ \footnote{The $\dashrightarrow$ notes that the map is only defined near the zero-section.}, meaning that we can hope to prove a result analogous to \cref{thm:cotangent_classification}. Since its proof relies on a linear interpolation of $1$-forms, we introduce the following notion.

\begin{definition}
    A neighbourhood $\mathcal{U} \subset \mathcal{L}$ of $L_0$ is called \textbf{Weinstein convex} (\textbf{W-convex}) if there is a Weinstein chart $φ: T^*L_0 \dashrightarrow X$ of $L_0$ such that $\varphi(\mathcal{W}) = \mathcal{U}$ for a subset $\mathcal{W} \subset \Omega^1(L_0)$ which is convex in the set of (closed) $1$-forms. Such a pair $(\symcal{U},φ)$ is called \textbf{Weinstein convex pair} (\textbf{W-convex pair})
\end{definition}

By the definition of the $C^1$-topology on $\mathcal{L}$, every neighbourhood of $L_0$ in $\mathcal{L}$ contains a W-convex neighbourhood. Note that W-convex sets are simply connected, hence the local Flux map \eqref{eq:localflux} makes sense on them.

\begin{proposition}
  \label{thm:local_classification}
  Let $\symcal{U}$ be W-convex.
  The map $\Flux_{\symcal{U}}$ classifies $\symcal{U}$ up to Hamiltonian isotopies contained in $\symcal{U}$. By this we mean that any $L, L' \in \mathcal{U}$ are Hamiltonian isotopic by a Hamiltonian isotopy through $\symcal{U}$ iff $\Flux_{\symcal{U}}(L) = \Flux_{\symcal{U}}(L')$.
\end{proposition}
\begin{proof}
...
\end{lemma}
\begin{proof}
  Let $φ: T^*L_0 \dashrightarrow X$ be a Weinstein neighbourhood and $α_1,…,α_d ∈ Ω_1(L_0)$ be a set of closed $1$-forms inducing a basis of $H¹(L_0;ℝ)$. Set 
  ...
  where we have chosen a small enough domain for this definition to make sense. Let $\mathcal{U} \subset \mathcal{L}$ be a simply connected neighbourhood of $L_0$. Let $V \subset H^1(L_0;\mathbb{R})$ be a small enough (simply connected) neighbourhood of the origin such that $v_{L_0}(V) \subset \mathcal{U}$. To compute $\Flux_{\mathcal{V}}(v_{L_0}(b))$, we may choose a Lagrangian isotopy $\Lambda_t$ with $\Lambda_1 = v_{L_0}(b)$ through Lagrangians of the form $\varphi(\Gamma_{\alpha})$. Since~$V$ is simply connected, the flux does not depend on that choice. By \cref{thm:graph_flux}, we obtain $\Flux_{\mathcal{V}}(v_{L_0}(b)) = \Flux \Lambda_t = [\sum_i b_i \alpha_i] = b$.
\end{proof}
...
\]
where $\Lambda_t = \pi^{-1}(\gamma(t))$ for $\gamma \colon [0,1] \rightarrow U$ a smooth path with $\gamma(0) = b_0$ and $\gamma(1) = b$. Note that, since $U$ is simply connected and $\Flux$ is invariant under homotopies of Lagrangian isotopies, the map $\mu_U$ is well-defined. 

\begin{proposition}
    The map $\mu_U$ defines a Hamiltonian $T^n$-action on $$
\end{proposition}


Let $\gamma \colon [0,1] \rightarrow B_0$ be a smooth path with $\gamma(0) = b$ and $[\gamma]$ its class in the universal cover $\Tilde{B}_0 \rightarrow B_0$. Let
    \[
        I \colon \Tilde{B}_0 \rightarrow H^1(L_b;\mathbb{R}), \quad
        I([\gamma]) = \Flux(\Lambda_t = \pi^{-1}(\gamma(t)))
    \]  
be the \emph{developing map} of $\pi$. 
