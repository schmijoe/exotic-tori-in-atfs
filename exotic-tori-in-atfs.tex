...
\begin{definition}
  An \textbf{ATF-corner} of $Δ$ is a pair $(x,kv)$ with $x ∈ ∂Δ$ a corner, $v ∈ ℤ²$ the primitive direction of a branch cut intersecting $∂Δ$ at $x$ pointing into $Δ$, and $k$ the number of nodes on that branch cut.

  The \textbf{mutation at $(x,kv)$} is given by the piecewise linear map $\overline{S}_{kv}$, also denoted by $\overline{S}_{(x,kv)}$, as in \cref{thm:change_branchcut} describing the change of branch cut, followed by a nodal slide to make the newly created branch cut negligibly small.
\end{definition}
...
\begin{proof}
  ...
  Let $Δ_{n+1} = \overline{S}_{u_i^n} Δ_n$ be the result of mutating $Δ_n$ at the left ATF-corner $u_{k_n}^n$ of $e_n$ (if $e_n$ has multiple ATF-corners at the left, we pick $(x,v)$ with the rightmost $v$).
  Let $u_1^{n+1},…,u_{n_c}^{n+1}$ be the corners of $Δ_{n+1}$, such that for $i ≠ k_n$ we have $\overline{S}_{u_{k_n}^n} u_i^n = u_i^{n+1}$ and $u_{k_n}^{n+1}$ be the new corner created by the mutation at $u_{k_n}^n$.\footnote{After a mutation we might loose a corner if the eigenline of $u_{k_n}^n$ intersects $∂Δ_n$ in another corner with the same eigenline. We assume wlog.\ that this doesn't happen, as the sequence $Δ_n$ must attain the minimum number of corners for all $n ≥N$ for some $N ∈ ℤ$. We then take $Δ_0=Δ_N$.}

  Note that by choice of $\overline{S}_{u_{k_n}^n}$, we have $e_n ⊂ e_{n+1}$.
  If $u_i^n$ is a corner with multiple branch cut directions, we have $ℓ(e_{n+1}) = ℓ(e_n)$, where $ℓ(e)$ denotes the affine length of an edge $e$, otherwise $ℓ(e_{n+1}) > ℓ(e_n)$.
  ...
  Let $a_n = \max_{1 ≤k ≤n_c'} \{g_n(u_k^n)\}$.
  Clearly we have $a_n ≤ a_{n+1}$ (we have $g_n(u_k^n) = g_{n+1}(u_k^{n+1})$ if $k ≠ k_n$ and $g_n(u_{k_n}^n) < g_{n+1}(u_{k_n}^{n+1})$), $a_n > ℓ_n$ and also $a_n < ℓ_m$ for some $m>n$ (this is because each $1 ≤ k ≤ n_c'$ must appear in $k_{m}$ for $m>n$).
  In particular $a_n → ℓ_∞$ and $a_n$ has a strictly monotone subsequence $a_{n_i}$.
  
  Picking $w_i = u_k^{n_i}$ with $k=k_{n_i-1}$, we have $a_{n_i} = g_{n_i}(w_i)$ (realizing the maximum).
  ...
  Since $g_{n_i}(w_i) > g_n(u_{k_n}^{n+1})$ for all $n<n_i-1$, none of the half-planes where $\overline{S}_{u_{k_n}^n}$ is not the identity contain $w_i$ (by convexity of $Δ$).

  Set $w'_i = u_k^{n_i}$ with $k=k_{n_i}$, and let $w_∞ ∈ ∂Δ_0$ be the limit point of $w_i$, and $w'_∞$ the limit point of $w'_i$, contained in the line spanned by $e_0$.
  Let $v_i$ be the primitive vector pointing along $w_i-w_i'$, and $v_∞ = w_∞ - w'_∞$.
  Let $f$ be the edge of $Δ_0$ containing infinitely many $w_i$ (and $w_∞$), and $x_f$ the vector pointing along $f$ in the clockwise direction, and let $x_e$ be the vector pointing along $e_0$ in the clockwise direction.
  Then $v_{i+1} = v_i + λx_f - μx_e$ for $λ,μ>0$.
  We have that $\det(x_f,v_∞)>0$, and so for $i$ big enough also $\det(x_f,v_i)>0$, and also $\det(v_i, x_e)>0$.
  So $\det(v_{i+1},v_i) > 0$. ANNNNND WE ARE DONE!
\end{proof}
