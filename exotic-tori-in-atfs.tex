...
The use of ATFs to construct exotic tori was pioneered by Vianna \cite{Via16,Via17}. Computing the displacement energy germ is particularly easy for almost toric fibres; firstly because every regular almost toric fibre has a natural versal deformation by \cref{cor:VDatffibres} and secondly because in an almost toric fibration, it is particularly easy to identify which standard torus a versal deformation is Hamiltonian isotopic to (step (2) in the above outline). Let us illustrate this using the most basic example of the Chekanov torus in $\mathbb{R}^4 = \mathbb{C}^2$.

Let $μ\colon ℂ² → ℝ²$ be the standard moment map on $ℂ$ given by $μ(z_1,z_2)=( \pi\abs{z_1}^2, \pi\abs{z_2}^2)$, with the moment polytope $Δ ⊂ ℝ²$ being the upper right quadrant. Its toric fibres are the so-called product tori, meaning products of standard circles in the plane. Using similar techniques as in \cref{sec:upper_bound,sec:lower_bound} we find that the displacement energy of product tori is
\begin{equation}
...
Let $x_1>0$. As a warm-up to the case of the Chekanov torus, let us compute the displacement energy germs of the product tori $T_{1,1}^1 = T(x_1,x_1)$ and $T = T(x_1,x_2)$ with $x_2 > x_1$. The former is the monotone Clifford torus (the notation $T^1_{1,1}$ is coherent with the one used in \cref{def:our_tori}) and the latter is a non-monotone product torus. We use \cref{thm:VDtoricfibre} to find that $b \mapsto T(x+b)$ is a versal deformation of $T(x)$ for any $x \in \text{int} \Delta$. Together with \eqref{eq:DE_prodtori}, this yields the displacement energy germs
\begin{equation}
    [e]_{T^1_{1,1}}(b) = x_1 + \text{min}\{ b_1,b_2 \}, \quad
    [e]_{T}(b) = x_1 + b_1,
\end{equation}
