...
\begin{theorem}
  ...
  \[ \eval{S_{T_k(a)}^e}_U (x,y) = a+\min\{x,x(1-kpq)+ykp²\}\todo{so oder so ähnlich…} \]
\end{theorem}
...
\begin{lemma}
  ...
  Let $(x,y) ∈ Δ_{B_{dpq}}$ not on the branch cut line. Then the displacement energy of $T = μ^{-1}(x,y)$ is equal to $x$.
\end{lemma}

\begin{proof}
  First Nodal Slide so \emph{everything is nice}. By \cite[Theorem 8.9]{evans2021atfs}, the nodal slide can be «local», so the displacement energy of $T$ is invariant. \todo{Do it better.}

  Using probes (which is possible since \emph{everything is nice}), we get the upper bound $e(T)≤x$.
  
  Since \emph{everything is nice}, after the nodal slide we also have that $b_1 = \symbf{H}_1(T) ≤ \symbf{H}_1(a_i)$ for all $ 1 ≤ i ≤ d$, so we can apply \cref{lem:lower_bound}, and to get the lower bound $e(T) ≥ x$.
\end{proof}
...
\begin{proof}[Proof of \cref{thm:bdpqexotic}]
  ...
  Then using a versal deformation \todo{whooo magic word}, we can identify a neighbourhood $U$ of $0 ∈ H¹(T_k(a);ℝ)$ with a neighbourhood $V ⊂ \overline{Δ_{B_{dpq}}}^{\text{reg}}$ of the basepoint $b = μ(T_k(a))$, where we identify points in $\overline{Δ_{B_{dqp}}}^{reg}$ with their preimages, which are Lagrangian tori.
  ...
  If it is below the branch cut ray, the mutation sends it to $(a,\frac{q}{p}a) + (kp²y -x(kpq -1), -kq²x + y(kpq +1))$, and the displacement energy is $a + -kq²x + y(kpq + 1))$, so we get the desired result.
\end{proof}
