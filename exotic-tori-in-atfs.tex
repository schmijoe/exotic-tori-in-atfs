...
\emph{Step 1 : }
    We start by performing a nodal trade at every toric vertex of $\Delta$.
    We define a sequence of ATF-mutations of $\Delta$.
    ...
    Set $\Delta_0 = \Delta$ and let $z_0$ be the ATF-corner associated to the left vertex of the distinguished edge $e_0 = e$ in case that vertex is simple.
In case that vertex is not simple, let $z_0$ be its ATF-corner containing the right-most node.
Set $\Delta_1 = \overline{S}_{z_0} \Delta_0$. Note that $\Delta_1$ has a unique edge $e_1$ contained in $O_x$, satisfying $e_0 \subset e_1$. This inclusion is strict if and only if the vertex at which we mutated was simple.
...
Let $z_1$ be the ATF-corner on the left vertex of $e_1$, defined similarly as $z_0 \in \Delta_0$. Iterating this construction, we obtain a sequence of ATF-base diagrams $\{\Delta_n\}_{n \in \mathbb{N}}$, distinguished edges $e_n \subset \Delta_n$ and adjacent ATF-corners $z_n \in e_n \subset \Delta_n$ such that $\Delta_{n+1} = \overline{S}_{z_n} \Delta_n$. See \cref{fig:stretch_iteration}.
    The distinguished edges are nested, $e_n \subset e_{n+1}$ and this inclusion is strict whenever the left vertex of $e_n$ is simple.
...
The labelling for $n=0$ is arbitrary and labels for $n > 0$ are recursively assigned as follows.
For every $k \in \{1,\ldots,N\}$ such that $u^n_k \neq z_n$ (i.e.\ for all vertices at which we do not mutate), set $u_k^{n+1} = \overline{S}_{z_n} u_k^n$. For $k$ with $u_k^n = z_n$ (this is the vertex at which we mutate), let $u_k^{n+1}$ be the vertex that is newly created by the mutation at $z_n$. 
...
Since this occurs at most a finite number of times, we can assume it does not occur at all by removing all redundant labels from $\Delta_{n_0}$ and setting $\Delta_0 = \Delta_{n_0}$ for some large enough $n_0$. 
    Let 
    \begin{equation}
        \label{eq:ln}
...
Up to relabelling, we can assume that the labels appearing infinitely many times are given by\footnote{Based on computer simulations, we suspect that $N'=2$. This means that our sequence of mutations $\{\Delta_n\}_{n \in \mathbb{N}}$ stabilizes to a sequence of mutations considered in \cite{EvaUrz21}.} $\{1,\ldots,N'\}$, for $N' < N$. 
    Let $n_1 \in \mathbb{N}$ be large enough such that $l_n \in \{1, \ldots,N'\}$ for all $n \geqslant n_1$, i.e.\ none of the labels appearing finitely many times in $\{l_n\}_{n \in \mathbb{N}}$ appear after the $n_1$-th mutation.

    Let $\{\Delta_n' = \Delta_{n + n_1}\}_{n \in \mathbb{N}}$ be the shifted sequence of mutations, starting at $\Delta_0' = \Delta_{n_1}$. The corresponding distinguished edges are denoted by $e_n' = e_{n_1 + n}$ and the labelled ATF-corners by $v_k^n = u_k^{n_1 + n}$. By definition, only the labels $\{1,\ldots,N'\}$ appear in its sequence of mutations of $\Delta_n'$ and they all appear infinitely many times.
\smallskip

\emph{Step 3 : } 
    We parametrize points on the boundary of the ATF-base diagrams $\Delta_n'$ by their integral affine arc length as follows. 
    Note that the right vertex $v_r$ of the distinguished edges $e_n'$ coincides for all $n$, since it lies in the half plane in which $\overline{S}_{z_n} = \id$ and hence $v_r \in \partial \Delta_n$ for all $n$. Let $P > 0$ be the integral affine perimeter of $\Delta_n'$, i.e.\ the sum of integral affine lengths of its edges.
Note that it is preserved by mutations and thus independent of $n$. Let 
    \[
        g_n \colon \partial \Delta_n' \rightarrow [0,P)
    \]
    be the function such that $g_n(x)$ measures the integral affine length of the clockwise arc in $\partial \Delta_n$ between $v_r$ and $x \in \partial \Delta_n$. The function $g_n$ is a bijection and continuous away from $v_r$. Note that, for every ATF-corner $u \in \Delta_n$,
    \begin{equation}
        \label{eq:gn_invariance}
        g_{n+1}(\overline{S}_{u}(x)) = g_n(x), \quad
        \forall x \in \partial \Delta'_n.
    \end{equation}
    Define
    \begin{equation}
        \label{eq:alphabeta}
        \alpha_n
        = \min_{1 \leqslant i \leqslant N'} g_n(v_i^n), \quad
        \beta_n
        = \max_{1 \leqslant i \leqslant N'} g_n(v_i^n).
    \end{equation}
    ...
    \emph{Claim : } Both sequences $\{\alpha_n\}_{n \in \mathbb{N}}$ and $\{\beta_n\}_{n \in \mathbb{N}}$ are monotone increasing and have strictly increasing subsequences. 
    
    Note that $\alpha_n = \ell(e_n')$ is equal to the integral affine length of the distinguished edge $e_n'$. Since $e_n' \subset e_{n+1}'$, the sequence $\{\alpha_n\}$ is monotone increasing.
The inclusion $e_n' \subset e_{n+1}'$ is strict if and only if the the vertex at which the mutation was performed was simple.
Since after a finite number of mutations, it is simple, the sequence $\{\alpha_n\}$ has a strictly increasing subsequence.
The sequence $\{\beta_n \}$ is monotone increasing since we have $g_{n+1}(v_k^{n+1}) \geqslant g_n (v_k^n)$. Indeed, this follows from \eqref{eq:gn_invariance} together with the fact that, at the $n$-step, we mutate at the vertex realizing the minimum $\beta_n$, meaning that, when mutating, the minimal value in $\{g_n(v_i^n)\}_{1 \leqslant i \leqslant N'}$ is eliminated and all others preserved, see also \cref{fig:stretch_iteration}. The sequence $\{\beta_n \}$ has a strictly increasing subsequence.
...
However, this would mean that the label of $n_2$ would appear only a finite number of times in the sequence \eqref{eq:ln}. This is a contradiction with the definition of $\Delta_n'$ and hence we have proved the claim.
    ...
    In particular, since both sequences are bounded, this means that they converge to $\alpha_{\infty}$ and $\beta_{\infty}$, respectively. \smallskip
...
\emph{Step 4 : }
    ...
    Furthermore, note that all $w_k'$ for large enough $k$ and $w_{\infty}$ are contained in a fixed edge $f \subset \Delta_0$. Let $x_f$ be the primitive vector defining $f$ (and pointing in the counter-clockwise direction). Similarly, let $x_e = (1,0)$ be the primitive vector defining the distinguished edge.  
    Let $v_k$ be the primitive vector pointing along $w_k'-w_k$, and $v_∞ = w_∞' - w_∞$.
    Then $v_{k+1} = v_k - λx_f + μx_e$ for $λ,μ>0$ and $k$ big enough.
     We have that $\det(v_k,x_f)>0$ (by convexity of the ATF-base diagram $Δ_n$), and also $\det(v_k,x_e)<0$.
    So $\det(v_{k+1},v_k) > 0$.
    ...
    Since $v_k$ is primitive, the Euclidean length $\norm{v_k}$ satisfies $\norm{v_k} → ∞$. The vector $v_∞$ is not parallel to $x_f$ (it would contradict the convergence of $w_k$), so $p_k = \det(x_f, -v_k) → ∞$, so $w_k$ is an ATF-corner of type $(d_k,p_k,q_k)$ for some $d_k,q_k$ in $Δ_{n_k}$, with $p_k → ∞$.

    Using \cref{thm:toric_embeddings} we get partial $B_{d_k,p_k,q_k}$-embedding for all $k ∈ \mathbb{N}$.
\end{proof}

\begin{proof}
    Set $Δ_0 = Δ$ and pick an edge $e_0$ of $Δ_0$. This is the edge that will be elongated by iterated mutations at an adjacent vertex.
