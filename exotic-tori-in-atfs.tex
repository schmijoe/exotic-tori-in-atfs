...
\begin{proof}[Proof of \cref{thm:toric_embeddings}]
    Recall from \cref{def:atf_corner} that an ATF-corner $u$ consists of a pair $u=(x,dv)$ given by a vertex $x$ and a multiple $dv$ of the branch cut direction. To simplify, we slightly abuse notation in this proof and denote by $u$ the ATF-corner, as well as the corresponding vertex $x$. Throughout the proof, we assume, possibly after performing nodal slides, that all branch cuts are negligibly small. \smallskip
...
\emph{Step 1 : }
    ...
    Set $\Delta_0 = \Delta$ and let $z_0$ be the ATF-corner associated to the left vertex of the distinguished edge $e_0 = e$ in case that vertex is simple. In case that vertex is not simple, let $z_0$ be its ATF-corner containing the right-most node. Set $\Delta_1 = \overline{S}_{z_0} \Delta_0$. Note that $\Delta_1$ has a unique edge $e_1$ contained in $O_x$, satisfying $e_0 \subset e_1$. This inclusion is strict if and only if the vertex at which we mutated was simple. We call $e_1 \subset \Delta_1$ the \emph{distinguished edge}, as well. Let $z_1$ be the ATF-corner on the left vertex of $e_1$, defined similarly as $z_0 \in \Delta_0$. Iterating this construction, we obtain a sequence of ATF-base diagrams $\{\Delta_n\}_{n \in \mathbb{N}}$, distinguished edges $e_n \subset \Delta_n$ and adjacent ATF-corners $z_n \in e_n \subset \Delta_n$ such that $\Delta_{n+1} = \overline{S}_{z_n} \Delta_n$. See \cref{fig:stretch_iteration}.
    The distinguished edges are nested, $e_n \subset e_{n+1}$ and this inclusion is strict whenever the left vertex of $e_n$ is simple. Note that after a finite number of mutations at a non-simple vertex, it becomes simple and thus the sequence $\{e_n\}_{n \in \mathbb N}$ contains a subsequence of edges which becomes strictly longer. \smallskip

\emph{Step 2 : }
    Let $u^0_1, \ldots , u^0_N$ be the ATF-corners of $\Delta_0$. The set $\{1,\ldots,N\}$ will serve as labels for the ATF-corners throughout the mutations. The labelling for $n=0$ is arbitrary and labels for $n > 0$ are recursively assigned as follows. For every $k \in \{1,\ldots,N\}$ such that $u^n_k \neq z_n$ (i.e.\ for all vertices at which we do not mutate), set $u_k^{n+1} = \overline{S}_{z_n} u_k^n$. For $k$ with $u_k^n = z_n$ (this is the vertex at which we mutate), let $u_k^{n+1}$ be the vertex that is newly created by the mutation at $z_n$. 
    In other words, every ATF-corner surviving mutation keeps its label and the ATF-corner at which we mutate cedes its label to the newly created ATF-corner. See \cref{fig:stretch_iteration} for an illustration in which the labels are represented by colours.
    Although the total number of ATF-nodes is constant throughout mutations, note that the number of ATF-corners may decrease if two opposite corner have the same eigenline\footnote{Consider for example any mutation of the square (the toric base of a monotone $S^2 \times S^2$) in the plane.}. This means that we may have $u_i^n = u_j^n$ for some $n$ and $i \neq j$, meaning that an ATF-corner carries a double label. Since this occurs at most a finite number of times, we can assume it does not occur at all by removing all redundant labels from $\Delta_{n_0}$ and setting $\Delta_0 = \Delta_{n_0}$ for some large enough $n_0$. 
    Let 
    \begin{equation}
        \label{eq:ln}
        l_n = \{ k \in \{1,\ldots,N\} \, \vert \, z_n = u_k^n\} 
    \end{equation}
    be the label of the ATF-corner at which the $n$-th mutation is performed. There are two types of labels - those appearing finitely many times in the sequence $\{l_n\}_{n \in \mathbb{N}}$ and those appearing infinitely many times. Up to relabelling, we can assume that the labels appearing infinitely many times are given by\footnote{Based on computer simulations, we suspect that $N'=2$. This means that our sequence of mutations $\{\Delta_n\}_{n \in \mathbb{N}}$ stabilizes to a sequence of mutations considered in \cite{EvaUrz21}.} $\{1,\ldots,N'\}$, for $N' < N$. 
    ...
    Let $\{\Delta_n' = \Delta_{n + n_1}\}_{n \in \mathbb{N}}$ be the shifted sequence of mutations, starting at $\Delta_0' = \Delta_{n_1}$. The corresponding distinguished edges are denoted by $e_n' = e_{n_1 + n}$ and the labelled ATF-corners by $v_k^n = u_k^{n_1 + n}$. By definition, only the labels $\{1,\ldots,N'\}$ appear in its sequence of mutations of $\Delta_n'$ and they all appear infinitely many times. \smallskip

\emph{Step 3 : } 
    We parametrize points on the boundary of the ATF-base diagrams $\Delta_n'$ by their integral affine arc length as follows. 
    Note that the right vertex $v_r$ of the distinguished edges $e_n'$ coincides for all $n$, since it lies in the half plane in which $\overline{S}_{z_n} = \id$ and hence $v_r \in \partial \Delta_n$ for all $n$. Let $P > 0$ be the integral affine perimeter of $\Delta_n'$, i.e.\ the sum of integral affine lengths of its edges. Note that it is preserved by mutations and thus independent of $n$. Let 
    \[
    ...
    Note that $\alpha_n = \ell(e_n')$ is equal to the integral affine length of the distinguished edge $e_n'$. Since $e_n' \subset e_{n+1}'$, the sequence $\{\alpha_n\}$ is monotone increasing. The inclusion $e_n' \subset e_{n+1}'$ is strict if and only if the the vertex at which the mutation was performed was simple. Since after a finite number of mutations, it is simple, the sequence $\{\alpha_n\}$ has a strictly increasing subsequence. The sequence $\{\beta_n \}$ is monotone increasing since we have $g_{n+1}(v_k^{n+1}) \geqslant g_n (v_k^n)$. Indeed, this follows from \eqref{eq:gn_invariance} together with the fact that, at the $n$-step, we mutate at the vertex realizing the minimum $\beta_n$, meaning that, when mutating, the minimal value in $\{g_n(v_i^n)\}_{1 \leqslant i \leqslant N'}$ is eliminated and all others preserved, see also \cref{fig:stretch_iteration}. The sequence $\{\beta_n \}$ has a strictly increasing subsequence. If it did not, then there would be an ATF-corner $v_0$ in all $\Delta_n'$ for $n \geqslant n_2$ for some large $n_2$ at which no more mutations are performed. However, this would mean that the label of $n_2$ would appear only a finite number of times in the sequence \eqref{eq:ln}. This is a contradiction with the definition of $\Delta_n'$ and hence we have proved the claim.
...
\end{figure}
  ...
  This is the distance from $u_{k_0}^0$ to furthest away (in terms of $g_n(\cdot)$) ATF-corner appearing in infinitely many mutations. We have $a_n ≤ a_{n+1}$, since $g_n(u_k^n) = g_{n+1}(u_k^{n+1})$ if $k ≠ k_n$ and $g_n(u_{k_n}^n) < g_{n+1}(u_{k_n}^{n+1})$.
