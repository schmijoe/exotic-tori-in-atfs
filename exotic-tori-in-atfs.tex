...
Note that $T(b_1,b_2)$ yields a well-defined torus up to Hamiltonian isotopy, i.e.\ it is independent of the nodal slides, see \todo{referenz, wahrscheinlich nicht der richtige ort für diesen kommentar.}.
...
\begin{proposition}
  Let $T=\symbf{H}^{-1}(b) ⊂ B_{dpq}$ with $b_2 ≠ 0$, $B_r(T) = \qty{p ∈ B_{dpq} \mid \inf{d(p,θ) < r \mid θ ∈ T}}$, $(M,ω)$ a tame symplectic manifold and $φ \colon B_r(T) → M$ a symplectic embedding. 
  
  If $\frac{πr²}{16} ≥ \max\qty{σ_S,μ_1(T)}$, then $e(M,φ(T)) ≥ μ₁(T)$.
\end{proposition}
