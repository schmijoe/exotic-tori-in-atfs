...
\begin{frame}{Almost Toric Fibrations}
  ...
  \begin{minipage}{4.5cm}
    A loop around a focus-focus singularity induces a monodromy map given by some conjugate of
    $\begin{pmatrix}
      1 & 1 \\
      0 & 1
    \end{pmatrix}$
...
%      \fill[blue,opacity=0.1] (-1,-1) rectangle (1,1);
%      \draw (0,0) node[cross] {};
%    \end{scope}
  ...
  \hspace{1cm}
  \begin{tikzpicture}
    \filldraw[thick,fill opacity=0.1]
      (-2,0) coordinate (A) --
      (0,-2) coordinate (B) --
      (2,2) coordinate (C) -- (A);
...
\begin{frame}{Fun Facts about Almost Toric Fibrations}
  The following induce fibred symplectomorphisms outside a compact neighbourhood: TODO Besser formulieren
% Nodal Slide
\begin{tikzcd}[ampersand replacement=\&]
...
\begin{frame}{Lower Bound on Displacement Energy: J-holomorphic curves}
  ...
  \begin{tikzcd}[ampersand replacement=\&]
    ...
    \end{tikzpicture} \&
    \begin{tikzpicture}[scale=0.7,baseline=0.35cm]
      \uncover<6->{
        \useasboundingbox (0,-1) rectangle (4,2);
        \fill[opacity=0.1] (0,-1) rectangle (4,2);
        \foreach \x in {0.2,0.4,...,4}
  ...
  $u\colon (D, ∂D) → (A,L)$ \pause and $H_1(A,L) = ⟨D_0⟩ ≅ ℤ$, \pause so $∫_u ω ≥ ∫_{D_0} ω = μ_1(L)$.
...
\begin{frame}{Lower Bound on Displacement Energy: J-holomorphic curves}
  ...
  \uncover<3->{By monotonicity we have that $∫_u ω ≥ ε$.}

  \uncover<4->{If $μ_1(L) ≤ \min\{ε, σ_S(X,J_X)\}$ then $e(L)≥ μ_1(L)$ still holds :)}
\end{frame}
